...
void write_super_block(void)
{
	...
	printf("%p %p %p %p %p %p %p %p", &(sb->s_ninodes), &(sb->s_nzones), &(sb->s_imap_blocks),
	     &(sb->s_zmap_blocks), &(sb->s_firstdatazone), &(sb->s_log_zone_size), &(sb->s_max_size),
			 &(sb->s_magic));
...
void write_maps(void)
{
	...
	//CLEAN_BLOCK_BUFFER();
  //map_blk[0] = 0x40; // first inode and data_zone used for root
